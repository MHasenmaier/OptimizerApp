...
function initDomReferences() {
    return {
        ...
        addNewTaskButton: document.getElementById("buttonTodoAddNewTask"),
...
async function todoPageLoaded() {
    ...
    addNewTaskButton.addEventListener("click", todoAddNewTask);
}
...
/**
 * Leitet den Benutzer zur leeren Task-Seite weiter,
 * mit Verknüpfung zur aktuellen Todo-ID
 */
function todoAddNewTask() {
    const params = new URLSearchParams(window.location.search);
    const todoId = params.get("id");

    if (!todoId || todoId === "-1") {
        console.warn("Es ist kein gültiges Todo geöffnet. Neue Task-Zuweisung abgebrochen.");
        return;
    }

    // Weiterleitung zur Task-Seite mit Referenz auf das aktuelle Todo
    location.href = `task.html?todoid=${todoId}`;
}
