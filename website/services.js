...
export function buildXmlFromObj(item, type) {
    ...
    console.error("services.js/buildXmlFromItem got invalid parameters and occurred an error.");
    return "";
}
...
export async function sendItemToDB(todoOrTask, xmlData) {
    ...
    const isNew = id === -1;
    let url = isNew
        ? `${urlToIndex}${todoOrTask}`
        : `${urlToIndex}${todoOrTask}?id=${id}`;

    if (isNew && window.location.search.includes("todoid=")) {
        const params = new URLSearchParams(window.location.search);
        const todoId = params.get("todoid");

        if (todoId) url += `?todoid=${todoId}`;
    }
    ...
    try {
        ...
        if (isNew && window.location.search.includes("todoid=")) {
            ...
            if (todoId) headers['X-Todo-Id'] = todoId;
        }
...
export async function countBegonnenItems(itemType) {
    ...
    if (itemType === "todo") {
        ...
        const allTasks = xmlToArray(await fetchActiveTasksFromDBXml(), "task");
 ...
 * @returns {Promise<string>}
 */
export async function fetchActiveTasksFromDBXml() {
    try {
        ...
        console.error("services.js/fetchActiveTasksFromDBXml: Fehler beim Laden der Tasks:", err);
        return "<tasks></tasks>";
    }
}
