<?php
	...
	{
		logDebug("getTodoById", "Funktion mit Parameter {$id} aufgerufen");
		$dbPDO = getPDO();
		if (!$dbPDO) {
		...
		try {
			...
			$stmt->execute(['id' => $id]);
			$retValue = $stmt->fetch(PDO::FETCH_ASSOC) ?: null;
			$debugRet = json_encode($retValue);
			logDebug("getTodoByID/try","try/retValue mit {$debugRet}");
			return $retValue;
		} catch (PDOException $e) {
	 ...
	 * @return array|null
	 */
	function createTask(string $inputXML): ?array
	{
		$dbPDO = getPDO();
		if (!$dbPDO) {
	...
	/** Gibt Anzahl an begonnenen Tasks zurück (Task mit Status === 4), NULL bei Fehler
	 * @return string|null
	 */
	 ...
	 */
	function convertXmlEntity(string $type, array $data): ?array
	{   //FIXME: anpassen, wenn ein leeres $data übergeben wird, wird ein string erkannt.
		$result = [];
		...
		switch ($type) {
			...
			case 'task':
				...
				foreach ($requiredFields as $field) {
					...
					if ($field === 'title' && trim($data[$field]) === '') {
						return null; // Titel darf nicht leer sein
					}

					//FIXME: Array to string conversion, wenn beschreibung leer bleibt - bei todo und task
					$result[$field] = ($field === 'status') ? (int)$data[$field] : (string)$data[$field];
				}
				...
				// Tasks für todo
				if ($type === 'todo' && array_key_exists('task', $data)) {
					if (is_array($data['task'])) {
						$result['task'] = array_map('intval', $data['task']);
					} else {
						$result['task'] = [(int)$data['task']]; // einzelner Task
					}
				}
	...
	/**
	 * Verknüpft einen Task mit einem Todo über die linktable
	 *
	 * @param int $taskId
	 * @param int $todoId
	 */
	function linkTaskToTodo(int $taskId, int $todoId): void
	{
		logDebug("linkTaskToTodo", "Wird ausgeführt mit task_id=$taskId, todo_id=$todoId");

		$db = getPDO();
		if (!$db) {
			logDebug("linkTaskToTodo", "getPDO() fehlgeschlagen");
			return;
		}

		try {
			$stmt = $db->prepare("INSERT INTO linktable (task_id, todo_id) VALUES (:taskId, :todoId)");
			$stmt->execute([
				'taskId' => $taskId,
				'todoId' => $todoId
			]);
			logDebug("linkTaskToTodo", "Verknüpfung erfolgreich: task_id=$taskId, todo_id=$todoId");
		} catch (PDOException $e) {
			logDebug("linkTaskToTodo", "Fehler beim Verknüpfen: " . $e->getMessage());
		}
	}
