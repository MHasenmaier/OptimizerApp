	...
	function handleGET(string $resource, array $query): bool
	{
		switch ($resource) {
			...
			case 'todo':
				...
				if ($todo === null) return statuscode(404, "Routing konnte Todo nicht finden");
				echo xmlFormatter($todo);
				return statuscode(200, "Routing hat Todo gefunden...");

			case 'task':
				if (!isset($query['id'])) return statuscode(400, "Task ID fehlt");
				$task = getTaskById((int)$query['id']);
				if ($task === null) return statuscode(404, "Routing konnte Task nicht finden");
				echo xmlFormatter($task, 'task');
				return statuscode(200, "Routing hat Task gefunden...");
			case 'todotasks':
	...
	function handlePOST(string $resource): bool
	{
		...
		switch ($resource) {
			...
			case 'task':
				...
				logDebug("routing/task", '$_GET Inhalt: ' . var_export($_GET, true));

				if ($task === null) return statuscode(500);

				$todoId = $_SERVER['HTTP_X_TODO_ID'] ?? null;
				if ($todoId !== null) {
					$taskId = (int)$task['id'];

					if ((int)$todoId > 0 && $taskId > 0) {
						linkTaskToTodo($taskId, (int)$todoId);
					}
				}

				echo xmlFormatter($task, 'task', false);
				logDebug("routing/task", "Neuer Task: " . var_export($task, true));
